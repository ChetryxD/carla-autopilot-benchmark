...
# ------------------------
# Video / Camera (UPDATED to baseline-style view, centered car, more surroundings)
# ------------------------
SAVE_VIDEO = True
...
# Baseline-style chase view like your screenshot
TPP_BACK = 9.0         # farther behind -> see more map/signals
TPP_UP = 4.2           # higher camera
TPP_PITCH_DEG = -18.0  # more downward view
CAM_FOV = 95           # wider FOV -> more surroundings
...
def run_one(client, traffic_count, target_speed_kmh):
    ...
    try:
        ...
        # video (UPDATED transform: y=0.0 + explicit rotation, centered car)
        if SAVE_VIDEO and cv2 is not None and np is not None:
            ...
            cam_tf = carla.Transform(
                carla.Location(x=-TPP_BACK, y=0.0, z=TPP_UP),
                carla.Rotation(pitch=TPP_PITCH_DEG, yaw=0.0, roll=0.0),
            )
